...
A feature does not meet this bar because it works. On setup, voice preview, Ready, alert history, permission recovery, and end-of-day summary, dogfooders must answer:

- Does this screen feel made for a busy shop counter rather than copied from a dashboard template?
- Is the status understandable at arm's length, with one obvious next action?
- Does the copy sound like a practical assistant, without blame, fake certainty, or technical jargon?
- Does motion confirm listening, testing, and recovery states? Remove motion that adds noise or slows a payment check.
- Do type size, contrast, touch targets, Tamil/Hindi/English wrapping, loading, empty, error, and reduced-motion states work on a low-cost real phone in a noisy setting?
- Are uncertainty, notification evidence, permissions, and unsupported messages stated plainly?
- When access is lost or parsing is uncertain, does the app explain the problem and recovery step without claiming a payment was received?

A screen fails dogfood if two merchants or reviewers call it template-like, confusing, cold, crowded, or untrustworthy. Record the exact screen and phrase, fix it, and retest on an installed build. No screenshot or score can raise experience confidence above 90% until this gate passes.
